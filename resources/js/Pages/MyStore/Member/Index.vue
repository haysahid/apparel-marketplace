...
const props = defineProps({
    members: Object as () => PaginationModel<UserEntity>,
});

const members = ref<PaginationModel<UserEntity>>({
    ...props.members,
    data: props.members.data.map((member) => ({
        ...member,
...
function getMembers() {
    router.get(route("my-store.member.index"), queryParams.value, {
        preserveState: true,
        preserveScroll: true,
        onSuccess: () => {
            getQueryParams();
...
const closeDeleteMemberDialog = (result = false) => {
...
function canEdit(member: UserEntity) {
    return false;
...
<template>
    <MyStoreLayout title="Anggota" :showTitle="true">
        <DefaultCard :isMain="true">
            <div class="flex items-center justify-end gap-4">
                <!-- <PrimaryButton
                    type="button"
                    class="max-sm:text-sm max-sm:px-4 max-sm:py-2"
                    @click="$inertia.visit(route('my-store.member.create'))"
                >
                    Tambah
                </PrimaryButton> -->
                <SearchInput
                    ...
                    >
                        <td>
                            {{
                                ...
                                (props.members.current_page - 1) *
                                    props.members.per_page
                            }}
                        </td>
                        <td>
                            <Link
                                :href="
                                    route('my-store.member.show', {
                                        member: member,
                                    })
                                "
                        ...
                        <td>
                            <AdminItemAction
                                ...
                                @edit="
                                    $inertia.visit(
                                        route('my-store.member.edit', {
                                            member: member,
                                        })
                                    )
                                "
                                @delete="openDeleteMemberDialog(member)"
                            />
                            <InfoTooltip
                                v-if="!canEdit(member)"
                                :id="`table-tooltip-hint-${member.id}`"
                                text="Anggota tidak dapat diubah atau dihapus"
                            />
                        </td>
                    </tr>
                </template>
            ...
            <!-- Pagination -->
            <div v-if="members.total > 0" class="flex flex-col gap-2 mt-4">
                <p class="text-xs text-gray-500 sm:text-sm">
                    Menampilkan {{ members.from }} - {{ members.to }} dari
                    {{ members.total }} item
                </p>
                <DefaultPagination
                    :isApi="true"
        ...
        <DeleteConfirmationDialog
            ...
            :title="`Hapus Anggota <b>${selectedMember?.name}</b>?`"
            @close="closeDeleteMemberDialog()"
            @delete="deleteMember()"
        />
