...
function getPaymentMethods() {
    axios
        ...
        .then((response) => {
            paymentMethods.value = response.data.result;
        })
        .catch((error) => {
            openErrorDialog("Gagal mendapatkan metode pembayaran");
        });
}
getPaymentMethods();

const shippingMethods = ref<ShippingMethodEntity[]>([]);
...
function getShippingMethods() {
    axios
        ...
        .then((response) => {
            shippingMethods.value = response.data.result;
        })
        .catch((error) => {
            openErrorDialog("Gagal mendapatkan metode pengiriman");
        });
}
getShippingMethods();
...
const submit = () => {
    ...
    if (orderStore.form.customer) {
        data["customer_id"] = orderStore.form.customer.id;
    } else if (
        orderStore.form.guest_name &&
        ...
        <div class="flex flex-col gap-y-4">
            ...
            <div class="flex flex-col gap-y-2">
                ...
                <div class="flex flex-row gap-2 mt-2">
                    <SecondaryButton class="w-full py-3" @click="emit('back')">
                        Kembali
                    </SecondaryButton>
                    <PrimaryButton
                        class="w-full py-3"
